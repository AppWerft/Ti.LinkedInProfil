...
/**
 *
 * LISessionManager manages the authorizations needed for an application to
 * access LinkedIn data and to view LinkedIn profiles.
 *
 * A typical usage flow is:
 *
 * LISessionManager.init(activity, Scope.build(LIPermission.R_BASICPROFILE),
 * callback, true);
 *
 * When callback.onAuthSuccess() is called, calls to
 * {@link com.linkedin.platform.APIHelper} to retrieve LinkedIn data or calls to
 * {@link com.linkedin.platform.DeepLinkHelper} can be made to view LinkedIn
 * profiles. {@link com.linkedin.platform.LISession#isValid()} should be used to
 * validate the session before making api calls or deeplink calls
 *
 */
public class LISessionManager {
	...
	/**
	 * Initializes LISession using previously obtained AccessToken The passed in
	 * access token should be one that was obtained from the LinkedIn Mobile
	 * SDK.
	 *
	 * @param accessToken
	 *            access token
	...
	/**
	 * Brings the user to an authorization screen which allows the user to
	 * authorize the application to access their LinkedIn data. When the user
	 * authorizes the application
	 * {@link com.linkedin.platform.listeners.AuthListener#onAuthSuccess()} is
	 * called. If the user has previously authorized the application,
	 * onAuthSuccess will be called without the authorization screen being
	 * shown.
	 *
	 * If there is no user logged into the LinkedIn application, the user will
	 * be prompted to login to LinkedIn, after which the authorization screen
	 * will be shown.
	 *
	 * Either this method or
	 * {@link com.linkedin.platform.LISessionManager#init(AccessToken)} must be
	 * called before the application can make API calls or DeepLink calls.
	 *
	 * @param activity
	 *            activity to return to after initialization
	 * @param scope
	 *            The type of LinkedIn data that for which access is requested.
	 *            see {@link com.linkedin.platform.utils.Scope}
	 * @param callback
	 *            listener to execute on completion
	 * @param showGoToAppStoreDialog
	 *            determines behaviour when the LinkedIn app is not installed if
	 *            true, a dialog is shown which prompts the user to install the
	 *            LinkedIn app via the app store. If false, the user is taken
	...
	/**
	 * This method must be called in the calling Activity's onActivityResult in
	 * order to process the response to
	...
	public void onActivityResult(Activity activity, int requestCode,
		...
		if (authListener != null && requestCode == LI_SDK_AUTH_REQUEST_CODE) {
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				authListener.onAuthError(new LIAuthError(
						LIAppErrorCode.USER_CANCELLED, "user canceled"));
			} else {
	...
	/**
	 * Clears the session. Calls to retrieve LinkedIn data or to view profiles
	 * will no longer work.
	 */
	...
	/**
	 * private implementation of LISession takes are of saving and restoring
	 * session to/from shared preferences
	 */
	private static class LISessionImpl implements LISession {
		...
		/**
		 * @return true if a valid accessToken is present. Note that if the
		 *         member revokes access to this application, this will still
		 *         return true
		 */
		@Override
		...
		private SharedPreferences getSharedPref() {
			return LISessionManager.sessionManager.ctx.getSharedPreferences(
					LI_SDK_SHARED_PREF_STORE, Context.MODE_PRIVATE);
		}

		private void save() {
			SharedPreferences.Editor edit = getSharedPref().edit();
			edit.putString(SHARED_PREFERENCES_ACCESS_TOKEN,
					accessToken == null ? null : accessToken.toString());
			edit.commit();
		}
